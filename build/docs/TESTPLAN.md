...
| 1.8 | Open `large.tar`, `docs.tar.gz`, `backup.7z`, and a `.deb` — **`~/INDIUM/build/package/out/indium_2.1.0-1_amd64.deb`**, which is INDIUM's own released package and the one `.deb` this machine is certain to hold. *(Corrected in PXX. This used to say "a `.deb` from `/var/cache/pacman/pkg/`" — that is **Arch's** package cache: 133 `.pkg.tar.zst` and **not one** `.deb`, because a `.deb` could never be in it. The step named a file that cannot exist here, and it was approved anyway, which is why the plan was scanned for its own broken references rather than trusted.)* | All four open and list. The `.deb` is read as a container, not refused — INDIUM reads it as the `ar` archive it is, and the round trip of opening the project's own shipped package is a fair test of §5's claim. | §5 |
