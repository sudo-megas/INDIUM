...
<p>
  ...
  <img alt="Version"      src="https://img.shields.io/badge/version-2.5.0-E95420?style=for-the-badge">
...
<p>
  ...
  <img alt="Debian package" src="https://img.shields.io/badge/Debian%20-6.92%20MB-772953?style=for-the-badge&logo=debian&logoColor=white">
</p>
...
Download `indium-2.5.0-1-x86_64.pkg.tar.zst` from the Releases page:

```sh
sudo pacman -U indium-2.5.0-1-x86_64.pkg.tar.zst
```

### 3.B Debian / Ubuntu

Download `indium_2.5.0-1_amd64.deb` from the Releases page:

```sh
sudo apt install ./indium_2.5.0-1_amd64.deb
```
...
`indium-2.5.0-1-x86_64.tar.gz` is the `.deb`'s binary with no packaging around it, so it carries
...
| **Settings** `,` | A small panel, three groups and no more: whether Extract should start out pointed into a subfolder, your bookmarks, and your recent files — the last with a **Clear list** that forgets all of them at once. It is a plain `settings.toml` you may edit by hand, and INDIUM respects what you write there. |
